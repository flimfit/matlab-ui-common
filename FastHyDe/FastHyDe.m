...
% ---------------------------------------------------------------------
    ...
    eigen_Y_bm3d=zeros(k_subspace,N);
    for i=1:k_subspace
        ...
        % denoise  with BM3D
        eigen_im = single(eigen_im);
        filt_r1_eigen_im = bm3d(eigen_im, sigma);
        filt_r2_eigen_im = da3d(eigen_im, filt_r1_eigen_im, sigma);
        filt_r2_eigen_im = double(filt_r2_eigen_im);

        eigen_Y_bm3d(i,:) = reshape(eigen_im *scale + min_x, 1,N);
    end

    % reconstruct data using denoising eigen images
